...
class TraslateRapidToPython( rapid_ast.NodeVisitor ):
    ...
    def __init__(self, out_filename="rapid_program.py", out_folder="run"):
        ...
        self.writemodule( "#this file is an automatic traslation of rapid code")
        self.writemodule( "import sys")
        self.writemodule( "sys.path.append('..')")
        self.writemodule("import pyrapid")
        self.writemodule("from collections import OrderedDict")
        self.writemodule("import copy")
        self.writemodule("import sys")

        
    def close(self, entrypoint=None):
        if not(entrypoint is None):
            self.writemodule("if __name__ == '__main__':" )
            ...
            if len(entrypoint_list)==2:
                ...
                procname = entrypoint
            self.writemodule("pyrapid.Symbols().GetSymbol(%s, modulename=%s).data({})"%(repr(procname), repr(modulename)))
            self.popModuleScope()
    ...
    def visit_AliasDefATS(self, node, **kwargs):
        ...
        if self.tmp_TypeDefAST_data is None:
            ...
            self.writemodule( "pyrapid.Symbols().AddGlobalSym(%s, pyrapid.Symbol.CreateDataType(pyrapid.Symbols().GetSymbol(symbolname=%s, modulename=%s, functionname=%s).data))" % (
             repr(aliasname), 
             repr(typename), repr(self.module_name), repr(self.function_name), ))
        else:
            pyrapid.Symbols().AddModuleSym(self.module_name, aliasname, pyrapid.Symbol.CreateDataType(newsym))
            self.writemodule( "pyrapid.Symbols().AddModuleSym(%s, %s, pyrapid.Symbol.CreateDataType(pyrapid.Symbols().GetSymbol(symbolname=%s, modulename=%s, functionname=%s).data))" % ( 
             repr(self.module_name), repr(aliasname), 
             repr(typename), repr(self.module_name), repr(self.function_name), ))
...
""" % (record_name, attributename, attributetype)
        ...
        if self.tmp_TypeDefAST_data is None:
            ...
            self.writemodule( "pyrapid.Symbols().AddGlobalSym(%s, pyrapid.Symbol.CreateDataType(rapid_%s))" % (
             repr(record_name), 
             record_name,))
        else:
            pyrapid.Symbols().AddModuleSym(self.module_name, record_name, pyrapid.Symbol.CreateDataType(eval("rapid_%s" % (record_name,))))
            self.writemodule( "pyrapid.Symbols().AddModuleSym(%s, %s, pyrapid.Symbol.CreateDataType(rapid_%s))" % (
             repr(self.module_name), repr(record_name), 
             record_name, ))
    ...
    def VarConstPersDeclaration(self, node, createvar, createvars, **kwargs):
        ...
        if len(varinstance_size)==0:
            getvar = createvar(pyrapid.Symbols().GetSymbol(vartype, self.module_name, self.function_name).data(varinstance_value))
            sgetvar = "%s(pyrapid.Symbols().GetSymbol(symbolname=%s, modulename=%s, functionname=%s).data(%s))" % ( createvars, repr(vartype), repr(self.module_name), repr(self.function_name), reprvarinstance_value )
        else:
            size = []
            ...
            getvar = createvar(   pyrapid.rapid_array(size, pyrapid.Symbols().GetSymbol(vartype, self.module_name, self.function_name).data, varinstance_value ) )
            sgetvar = "%s( pyrapid.rapid_array(%s, pyrapid.Symbols().GetSymbol(symbolname=%s, modulename=%s, functionname=%s).data, %s) )" % ( createvars, str(size), repr(vartype), repr(self.module_name), repr(self.function_name), reprvarinstance_value )
    ...
    def visit_StatementAST(self, node, error_statement=False, undo_statement=False, **kwargs):
        ...
        if error_statement or undo_statement:
            ...
            self.writemodule("while retry_stantment:")
            with self.ModuleScope():
                self.writemodule("retry_stantment = False")
                self.writemodule("try:")
                with self.ModuleScope():
                    self.visit(node.statement, **kwargs)
                self.writemodule("except pyrapid.RapidException, ex:")
                with self.ModuleScope():
                    self.writemodule("error_manager_call = pyrapid.Symbols().GetSymbol(symbolname=%s, modulename=%s).error_call" % ( repr(self.function_name), repr(self.module_name) ))
                    self.writemodule("if not(error_manager_call is None):")
                    with self.ModuleScope():
                        self.writemodule("error_manager = error_manager_call(ex)")
                        self.writemodule("if error_manager == 1:")  # retry
                        with self.ModuleScope():
                            self.writemodule("retry_stantment = error_manager")  # retry
                        self.writemodule("elif error_manager == 2:")  # trynext
                        with self.ModuleScope():
                            self.writemodule("pass")  # trynext
                        ...
                        with self.ModuleScope():
                            ...
                            with self.ModuleScope():
                                self.writemodule("undo_manager_call()")
    ...
    def visit_RaiseStatementAST(self, node, **kwargs):
        ...
        if node.error_number is None:
            ...
            self.writemodule('raise pyrapid.RapidException(%d,"unknown")'%(node.error_number,))
    ...
    def visit_RetryStatementAST(self, node, **kwargs):
        if not (node.coord is None):
            self.writemodule("# Line %s"%(str(node.coord),))
        self.writemodule("return 1")
        
    def visit_TrynextStatementAST(self, node, **kwargs):
        if not (node.coord is None):
            self.writemodule("# Line %s"%(str(node.coord),))
    ...
    def visit_ProcedureCallAST(self, node, **kwargs):
        ...
        if procedure_name=="WaitUntil":
            ...
            self.simple_writemodule()
            with self.ModuleScope():
                self.writemodule("time.sleep(0.1)")
        else:
    ...
    def visit_ProcedureDeclarationAST(self, node, **kwargs):
                ...
                for ik,iv in self.params.iteritems():
                    ...
                    if vartype=="switch":
                        ...
                        try:
                            ...
                            raise Exception("error probably not defined symbol %s (module %s, function %s)"%(ptype, self.module_name, self.function_name))
        ...
        if self.tmp_outineDeclarationATS_data is None:
            self.writemodule("# rapid2py.py line:%d"%( pyrapid.lineno(), ))
            self.writemodule( "pyrapid.Symbols().AddGlobalSym(%s, pyrapid.Symbol.CreateProcedure(%s, rapid_proc_%s, error_rapid_proc_%s, undo_rapid_proc_%s), %s)" % (
             repr(self.function_name), 
             params, self.function_name, self.function_name, self.function_name, 
             repr(self.module_name),))
        else:
            self.writemodule("# rapid2py.py line:%d"%( pyrapid.lineno(), ))
            self.writemodule( "pyrapid.Symbols().AddModuleSym(%s, %s, pyrapid.Symbol.CreateProcedure(%s, rapid_proc_%s, error_rapid_proc_%s, undo_rapid_proc_%s))" % (
             repr(self.module_name), repr(self.function_name), 
             params, self.function_name, self.function_name, self.function_name, ))
    ...
    def visit_FunctionDeclarationAST(self, node, **kwargs):
        ...
        if self.tmp_outineDeclarationATS_data is None:
            ...
            pyrapid.Symbols().AddModuleSym(self.module_name, self.function_name, pyrapid.Symbol.CreateFunction(self.params, returntype, _rapid_func_, _error_rapid_func_, _undo_rapid_func_))
        ...
        if self.tmp_outineDeclarationATS_data is None:
            self.writemodule( "pyrapid.Symbols().AddGlobalSym(%s, pyrapid.Symbol.CreateFunction(%s, %s, rapid_func_%s, error_rapid_func_%s, undo_rapid_func_%s), %s)" % (
                                                              repr(self.function_name),
                                                              params, sreturntype, self.function_name, self.function_name, self.function_name, 
                                                              repr(self.module_name),))
        else:
            self.writemodule( "pyrapid.Symbols().AddModuleSym(%s, %s, pyrapid.Symbol.CreateFunction(%s, %s, rapid_func_%s, error_rapid_func_%s, undo_rapid_func_%s))" % (
                                                              repr(self.module_name), repr(self.function_name), 
                                                              params, sreturntype, self.function_name, self.function_name, self.function_name, ))
...
if __name__ == '__main__':
    ...
    parser = OptionParser()
    parser.add_option("-f", "--file", action="store", type="string", dest="filename", default="rapid_program.py",
                        help="output filename", metavar="FILE")
    parser.add_option("-d", "--directory", action="store", type="string", dest="directory", default="run",
                        help="output directory", metavar="DIRECTORY")
    parser.add_option("-e", "--entrypoint", action="store", type="string", dest="entrypoint",
                        help="procedure to call as entry point", metavar="PROCNAME")
                        
    parser.add_option("-i", "--infile", action="append", type="string", dest="infiles",
                        help="input file to convert", metavar="FILE")
                        
    parser.add_option("-s", "--nostandard", action="store_false", dest="standard", default=True,
                      help="not include standard symbols")
                      
    parser.add_option("-r", "--run", action="store_true", dest="run", default=False,
                      help="run entry point")
    ...
    traslate = TraslateRapidToPython(out_filename=options.filename, out_folder=options.directory)
    try:
    ...
    traslate.close(entrypoint)
    
    if options.run:
        import subprocess
        a=subprocess.Popen(["python", os.path.basename(traslate.file_module_name)], shell=False, cwd=os.path.dirname(traslate.file_module_name))
        a.wait()
        print "End"
        #
        #
